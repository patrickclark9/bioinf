...
- La QSAR non è la verità, ma aiuta a capire ciò che accade, riduce il numero di esperimenti da eseguire
- Non sostituisce il lavoro di laboratorio
- Non esiste il modello perfetto
	- Dipende da dati, descrittori e tutti gli altri fattori
...
Significa che, all'interno di questa serie chimica e dominio, incrementare logP è associato a un incremento nell'attività predetta — **non** che l'incremento di logP _causi_ automaticamente l'incremento di attività in generale.
...
- $K$ → costante di Equilibrio/rate per il composto sostituito
- $K_0$ → costante di Equlibrio/rate per il composto di riferimento
- $\rho$ → mette in relazione uno scaffold/equilibrio con un acido benzoico di riferimento; descrive quanto forte è la risposta di una particolare reazione all'effetto di un sostituente elettronico. Se $\rho$ è minore di 0 la reazione è accelerata da gruppi elettron donatori che stabilizzano una carica positiva in sviluppo (stato di transizione carbocatione-like). Se maggiore di 0, la reazione è favorita da gruppi electron-withdrawing e si forma una carica negativa
- $\sigma$ → descrittore che quantifica l'effetto di un sostituente sulla dissociazione per effetto induttivo e mesomerico: positivo per gruppi accettori di elettroni (-NO2, -CN, -Cl), negativo per gruppi donatori (-NH2, -OCH3, -CH3). Descrive come un sostituente cambia il carattere elettronico di una molecola rispetto all'idrogeno $\sigma_H = 0$ .
    - _Effetto mesomerico/risonanza_ → Effetto elettronico osservato nei legami $\pi$ (strutture di risonanza). La delocalizzazione degli elettroni, in risonanza con altri atomi, quindi la condivisione della carica negativa causa effetti differenti. La presenza di lone-pair in risonanza con gli altri elettronico produce un effetto mesomerico positivo $+M$, quando un gruppo dona densità elettronica. Se un gruppo preleva densità elettronica si ha effetto mesomerico negativo $-M$
    - _Effetto Induttivo_ -> Effetto elettronico osservato nei legami $\sigma$, dovuto alla differenza in elettronegatività degli atomi legati. Se l'atomo dona densità elettronica, ha un effetto positivo $+I$, se invece preleva densità elettronica ha effetto negativo $-I$. Atomi con elevata elettronegatività hanno effetti induttivi più elevati, aumentando la forza dell'acido
L'effetto mesomerico è tipicamente quello dominante
...
I sostituenti orto- vengono esclusi perchè entrano in gioco gli effetti sterici che hanno impatto sul termine elettronico.
...
La riformulazione in forma logaritmica venne fatta in seguito cosicchè la formulazione di Free-Wilson potesse essere utilizzata in combinazione con i parametri di Hansch, in cui alcuni termini sono variabili indicatore, altri sono descrittori fisico-chimici di Hansch.
Non necessita di dati esterni come l'analisi di Hansch, in cui servono valori tabulati.
Più limitante perchè non lavora bene in estrapolazione e non riesce ad analizzare combinazioni di sostituenti non presenti nel dataset.
Non spiega perchè un sostituente funziona, solo che statisticamente impatta l'attività.
