...
- Proprietà chimico-fisiche e indici topologici (MW, lipofilia, ecc.) — riducibili mediante PCA.
	- I descrittori 1D sono spesso fortemente correlati, e fornire tutti questi descrittori ridondanti al modello può causare problemi (overfitting). La PCA aiuta a risolvere questo problema andando a comprimere la descrizione dei dati ed elimina il rumore di fondo. Le componenti sono inoltre ortogonali, quindi l'informazione contenuta in ogni componente è unica. Raggruppando le variabili insieme ci permette di cercare le proprietà che effettiviamente hanno un impatto sull'attività biologica
