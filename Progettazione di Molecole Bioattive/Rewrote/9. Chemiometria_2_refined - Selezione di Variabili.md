...
Gli algoritmi genetici sono algoritmi di ricerca/ottimizzazione che emulano i processi biologici di ricombinazione genetica, mutazione e selezione per generare possibili soluzioni. Sono metodi di ottimizzazione stocastica, caratterizzati dalla capacità di esplorare diverse regioni dello spazio parametrico.
...
| Fitness/Risposta | Qualità del modello               |
...
2. Ogni cromosoma viene valutato in relazione alla risposta prodotta. La fitness/rispostà viene valutata attraverso score quali $R^2$, Cross-Validation Score, $RMSE$.

3. Viene selezionato e mantenuto il modello più performante


    ![[Pasted image 20260809090601.png]]
    
4. **Crossing-over** — coppie di cromosomi vengono ricombinate per formare nuovi individui.![[Pasted image 20260809093935.png]]
    
5. **Mutazione** — il contenuto genico del cromosoma viene alterato in maniera casuale. `[1 0 1 1 0] → [1 1 1 1 0]`
    
6. L'algoritmo prosegue finché:
...
Ogni modello corrisponde a selezione un sottoinsieme di dimensioni nello **spazio chimico**, andando a cercare dove l'attività è meglio spiegata

**Vantaggi**:
- È in grado di gestire grossi spazi di ricerca
- Trova combinazioni non ovvie
- Evita minimi locali
- In grado di ottimizzare ogni metrica
**Svantaggi**
- Nessuna garanzia di ottimo globale
- Computazionalmente dispendioso
...
Il GP ricerca lo spazio delle equazioni, non solo delle variabili, sfruttando le rappresentazioni ad albero.
Gli step sono simili, ma al posto di generare cromosomi sotto forma di stringa binaria di inclusione/esclusione, vengono generate equazioni casuali, ed ad ogni step si memorizzano le equazioni con le prestazioni migliori.
Le mutazioni avvengono rimpiazzando parte dell'albero con nuove equazioni o aggiungendo o rimuovendo nodi.
È in grado di modellare relazioni non lineare, e non è necessario predefinire la struttura dell'equazione. Più flessibile dei modelli di regressione.
